...
st.caption("CatBoost classification with native categorical handling, missing-value support, and ranked probabilities.")
...
@st.cache_resource(show_spinner="Training attack-type model...")
def get_model(frame: pd.DataFrame) -> ModelResult:
    return train_classifier(frame, features, "attacktype1_txt")
...
if submitted:
    st.success(f"Predicted attack type: {result.pipeline.predict(input_df)[0]}")
